...
def main1(cuex,cuey,objx,objy,hitpointxs,hitpointys, values1):
    ...
    for i in range(0,6):
        ...
        if cue_obj_hole1>90:
            ...
            cue_obj_holeangle.append(cue_obj_hole1)
    main1obstacles=[]
    main1obstacles=target_hole(hitpointxs,hitpointys)
    way1scores=[]
    for i in range(0,6):
...
# 找到小於或等於0的最大分數
    if non_positive_scores:
        ...
        print("沒有小於或等於0的分數")
        main2(cuex,cuey,objx,objy, hitpointxs, hitpointys)

def main2(cuex,cuey,objx,objy, hitpointxs, hitpointys):
    cue_obj_diss=[]
    cue_objvxs=[]
    cue_objvys=[]
    obj_tar_diss=[]
    ...
    print(all_angle2)
    main2obstacles2=[]
    main2obstacles2=target_hole(hitpointxs,hitpointys)
...
def target_hole(hitpointxs, hitpointys):
    obstacles = []
    for i in range(0,6):
        count = 0 
...
def generateballs(numberofballs, r):
    ...
    # generate cue ball
    cuex = random.randint(x1+r,x1+width-r)
    cuey = random.randint(y1+r,y1+height-r)
...
def main(ballx_set, bally_set, ballcount):
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    print(route)
                    if route==1:
                        main1(cuex,cuey,ballx_set[0],bally_set[0], hitpointxs,hitpointys, values1)
                    else:
                        main2(cuex,cuey,ballx_set[0],bally_set[0], hitpointxs,hitpointys)
...
if __name__ == '__main__':
    # balls, ballcount = generate(8)
    cuex, cuey, ballx_set, bally_set, ballcount = generateballs(8, radius)
    # ballx_set = [ball.x for ball in balls]#read the information from balls 
    # bally_set = [ball.y for ball in balls]
    # color=[ball.color for ball in balls]
    # for ball in balls:
    ...
    # cue,cuex,cuey=oneball(WHITE)
    # print(balls)
    main(ballx_set, bally_set, ballcount)
